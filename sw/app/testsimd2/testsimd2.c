...
int main(void)
{
    ...
    //intrisic type 
    uintXLEN_t i_elements[2] = {0xaa0200ff, 0xaa0300ff};
    ...
    //Intrisic function call 
    uintXLEN_t i_result = __rv_add8(i_elements[0], i_elements[1]);
    char buffer[70];
    //snprintf(buffer, 55, "Y[7:0] = %u,Y[15:8] = %u, Y[23:16] = %u, Y[31:24] = %u", octet1, octet2, octet3, octet4);
    //snprintf(buffer, 70, "asmY = %d", result);
    snprintf(buffer, 70, "intrisicY = %d", i_result);
